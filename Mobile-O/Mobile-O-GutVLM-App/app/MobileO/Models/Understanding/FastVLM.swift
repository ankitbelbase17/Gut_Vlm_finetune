...
public class FastVLMProcessor: UserInputProcessor {
    ...
    /// Number of image feature tokens the vision encoder emits for a given
    /// preprocessed image size. A single `<image>` placeholder is expanded into
    /// this many image tokens so they splice 1:1 with the vision encoder patches.
    private func imageTokenCount(for imageTHW: THW) -> Int {
        let patchSize = config.patchSize
        var n = (imageTHW.h / patchSize) * (imageTHW.w / patchSize) + config.numAdditionalImageTokens
    ...
    public func prepare(prompt: UserInput.Prompt, imageTHW: THW?) -> String {
        // --- GutVLM raw-prompt passthrough -------------------------------------
        // Our VQA / hallucination tasks build the *exact* Qwen chat string used
        // during finetuning (see GutVLMModel / GutVLMPrompts). When the caller
        // already supplies a fully-formatted prompt (it contains role markers),
        // do NOT re-wrap it with a generic "You are a helpful assistant" system
        // message or extra <|im_end|> turns - just expand the single `<image>`
        // placeholder into image tokens and return it verbatim. This keeps the
        // on-device prompt byte-identical to training, which matters for the
        // small 0.5B medically-finetuned model.
        if case .text(let text) = prompt, text.contains("<|im_start|>") {
            guard let imageTHW else { return text }
            let imageTokens = String(repeating: config.imageToken, count: imageTokenCount(for: imageTHW))
            // Replace only the first `<image>` (our prompts contain exactly one).
            if let range = text.range(of: config.imageToken) {
                return text.replacingCharacters(in: range, with: imageTokens)
            }
            return text
        }
        // --- Default chat-template behaviour (unchanged) -----------------------
