...
final class ModelDownloadManager: NSObject {
    ...
    // TODO: replace with YOUR HuggingFace repo containing the exported
    // understanding models (vision_encoder.mlpackage + llm/). See README.
    private static let repo = "ankitbelbase034/Mobile-O-GutVLM-iOS"
    private static let baseURL = "https://huggingface.co/\(repo)/resolve/main"
    // Understanding-only: we ship ONLY the vision encoder + LLM. The DiT / VAE /
    // connector generation components are not used and not downloaded.
    static let coreMLComponents = ["vision_encoder"]
    ...
    func hasSufficientDiskSpace() -> Bool {
        let requiredBytes: Int64 = 3_000_000_000 // ~3 GB for download + compilation (understanding-only)
    ...
    func startDownload() {
        ...
        // Understanding-only: vision encoder (FP16) + LLM (8-bit) ~ 1.5-1.8 GB.
        if totalBytes == 0 {
            totalBytes = 1_800_000_000 // ~1.8 GB estimate
        }
